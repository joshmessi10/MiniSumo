...
int sidef = 0; //sensor proximidad frontal QTR1 A2
...
void robot_buscando(){ 
    ...
    if(adci <= limiteb1 || adcd <= limiteb2){
      ...
      led2off;
      led3on;
      if(sharpi<maxDistanceBig && sharpd<maxDistanceBig && sidef<maxDistanceMini){
          motors(95,65);
...
void robot_ataque(){
  ...
  if(adci <= limiteb1 || adcd <= limiteb2){
    ...
    if(sidef<maxDistanceMini && adci >= limiten1 && adcd >= limiten2){
        led1on;
        led2on;
        led3on;
        motors(255,255);
        lecturaSensores();
    }else if(sharpi<maxDistanceBig && sidef>maxDistanceMini && sharpd>maxDistanceBig && adci >= limiten1 && adcd >= limiten2){
        led1on;
        led2on;
        led3off;
        motors(255,60);
...
void robot_ataqueS(){
  ...
  if(adci <= limiteb1 || adcd <= limiteb2){
    ...
    if(sidef<maxDistanceMini && adci >= limiten1 && adcd >= limiten2){
      ...
      st_robot = BUSCANDO_EV;
    }
  }
  
}

void robot_ataqueSEv(){
